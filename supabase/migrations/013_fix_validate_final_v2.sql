-- Migration 013: Final fix for validate_apartment_credentials
-- The SELECT INTO needs table prefix on ALL columns, not just WHERE clause
...
DECLARE
  v_apartment RECORD;
BEGIN
  -- CRITICAL: Use table alias 'a.' for ALL columns in SELECT to avoid ambiguity
